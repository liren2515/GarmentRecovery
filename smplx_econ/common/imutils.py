...
from econ.common import constants
...
def process_image_cloth3D(img_file, mask_file, hps_type, single, input_res, detector):

    img_raw, (in_height, in_width) = load_img(img_file)
    mask_raw, _ = load_img(mask_file)
    tgt_res = input_res * 2
    ...
    try:
        boxes_anno = np.where(mask_square>200)
        y_min, x_min, y_max, x_max = boxes_anno[0].min(), boxes_anno[1].min(), boxes_anno[0].max(), boxes_anno[1].max()
        #print(x_min, y_min, x_max, y_max)
        boxes_anno = np.array([[x_min, y_min, x_max, y_max]]).astype(np.float32)
        masks_anno = mask_square
    except:
        return None, False

    M_crop = get_affine_matrix_box(boxes_anno, input_res, input_res)
    ...
    for idx in range(len(boxes_anno)):

        img_square_rgba = torch.cat([
            img_square.squeeze(0).permute(1, 2, 0),
            torch.tensor(mask_square).unsqueeze(-1)
        ],
                                    dim=2)
        ...
        img_rembg = img_crop
        img_mask = (img_crop[:, :, [3]]>200)#.astype(float)/255
        img_mask = remove_floats(img_mask)
        
        #print(img_rembg.shape, img_mask.shape, img_mask.max())
        #cv2.imwrite('img_rembg.png', img_rembg.astype(np.uint8))
        #cv2.imwrite('img_mask.png', img_mask.astype(np.uint8)*255)
        #sys.exit()
...
def process_image_landmarks_body(img):

    #img_rembg = img
    #img_mask = (img[:, :, [3]]>200)#.astype(float)/255
    #img_mask = remove_floats(img_mask)
    
    #print(img_rembg.shape, img_mask.shape, img_mask.max())
    #cv2.imwrite('../tmp/img_rembg.png', img_rembg.astype(np.uint8))
    #cv2.imwrite('../tmp/img_mask.png', img_mask.astype(np.uint8)*255)

    #img_np = (img_rembg[..., :3] * img_mask).astype(np.uint8)
    img_np = img

    landmarks = get_keypoints(img_np)
    landmarks_body = landmarks['body']
    #print(landmarks_body.shape, type(landmarks_body))
    #sys.exit()
