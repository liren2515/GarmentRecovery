...
def sewing_front_back(mesh_pattern_f, mesh_pattern_b, mesh_atlas_f, mesh_atlas_b, labels_f, labels_b, body_zero, z_offset=0.001):
    ...
    if mesh_pattern_f.vertices[path_seam2_f[0], 0] > mesh_pattern_f.vertices[path_seam2_f[-1], 0]: # left to right
        path_seam2_f = path_seam2_f[::-1]
    if mesh_pattern_b.vertices[path_seam2_b[0], 0] > mesh_pattern_b.vertices[path_seam2_b[-1], 0]:
    ...
    path_seam2_f_collar, path_seam2_f_sleeve = mesh_reader.catch_collar_vertices_v2(mesh_atlas_f, path_seam2_f, body_zero)
    path_seam2_b_collar, path_seam2_b_sleeve = mesh_reader.catch_collar_vertices_v2(mesh_atlas_b, path_seam2_b, body_zero)
    ...
    faces_seam2_sleeve = mesh_reader.triangulation_seam_v2(mesh_atlas_f, mesh_atlas_b, path_seam2_f_sleeve, path_seam2_b_sleeve, idx_offset, reverse=False)
    faces_seam2_collar = mesh_reader.triangulation_seam_v2(mesh_atlas_f, mesh_atlas_b, path_seam2_f_collar, path_seam2_b_collar, idx_offset, reverse=False)

    if len(faces_seam2_collar) == 0:
        faces_sewing = [mesh_atlas_f.faces, mesh_atlas_b.faces + idx_offset, faces_seam1, faces_seam2_sleeve]
    else:
        faces_sewing = [mesh_atlas_f.faces, mesh_atlas_b.faces + idx_offset, faces_seam1, faces_seam2_sleeve, faces_seam2_collar]
    ...
    idx_seam3_v = np.concatenate((idx_seam3_v_f, idx_seam3_v_b), axis=0)
...
def reconstruct(cnn_regressor, model_isp, latent_codes, images_input, mask_normal, smpl_related, uv, verts_uv_cano_mean, trans, scale, transforms, body_smpl, save_path):
    ...
    with torch.no_grad():
        ...
        mesh_atlas_fl, mesh_atlas_bl, mesh_pattern_fl, mesh_pattern_bl, label_f, label_b = reconstruct_pattern_with_label(model_isp, lat_code_opt, uv_vertices_256, uv_faces_256, edges_256, using_repair=True)
        ...
        if len(idx_collar_v) != 0:
            mesh_atlas_sewing = remove_collar(mesh_atlas_sewing, idx_collar_v)
            mesh_atlas_sewing = smooth_boundary(mesh_atlas_sewing)
        mesh_atlas_sewing = repair_pattern(mesh_atlas_sewing)
        mesh_atlas_sewing.export(os.path.join(save_path, 'mesh_atlas_sewing.obj'))

    #######################################################
    ''' step 3: optimize deformation prior '''
    #######################################################
    idx_sleeve_v = select_sleeve(mesh_atlas_sewing, body_zero)
    sleeve_edges = select_sleeve_boundary(mesh_atlas_sewing, set((np.arange(len(mesh_atlas_sewing.vertices))[idx_sleeve_v]).tolist()))
